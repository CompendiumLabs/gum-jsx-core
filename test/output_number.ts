...
assert.equal(number(1e-16), '0')
assert.equal(number(-0), '0')
assert.equal(output_number_formatter(3)(1 / 3), '0.333')
assert.equal(output_number_formatter(3)(123.45678), '123.457')
assert.equal(output_number_formatter(3)(0.00123456), '0.001')
assert.equal(output_number_formatter(3)(-123.45678), '-123.457')
assert.equal(output_number_formatter(3)(-0.0001), '0')
assert.equal(output_number_formatter(3)(1.2), '1.2')
assert.equal(output_number_formatter(0)(123.6), '124')
assert.equal(output_number_formatter(0)(-123.6), '-124')
assert.equal(output_number_formatter(100)(1e-100), '1e-100')
assert.equal(output_number_formatter(3)(1e21), '1e+21')
assert.equal(output_number_formatter('full')(0.1 + 0.2), '0.30000000000000004')
assert.equal(output_number_formatter('full')(1e-16), '1e-16')
for (const precision of [-1, 101, 1.5, NaN, Infinity]) {
...
assert.match(svg, /d="M0\.3 0L0\.3333333333 0"/)
assert.match(svg, /stroke-width="0\.3"/)
assert.match(render_svg(fragment, { precision: 3 }), /d="M0\.3 0L0\.333 0"/)
assert.match(render_svg(fragment, { precision: 0 }), /d="M0 0L0 0"/)
assert.match(render_svg(fragment, { precision: 'full' }), /M0\.30000000000000004 0L0\.3333333333333333 1e-16/)
assert.throws(() => render_svg(fragment, { precision: -1 }), /precision/)
