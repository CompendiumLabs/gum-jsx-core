...
const tests: Record<string, () => void> = {
  ...
  'radius corner overrides and side precedence are independent of object order'() {
    ...
    for (const radius of [entries, reversed]) {
      assert.deepEqual(radius_of(pass.layout(new Rect({ border_radius: radius, font_size: px(10) }), fixed)),
        { tl: zero, tr: { x: 10, y: 5 }, bl: { x: 5, y: 5 }, br: { x: 20, y: 20 } })
    }
    assert.deepEqual(radius_of(pass.layout(new Rect({ border_radius: {} }), fixed)), corners({}))
  },
  ...
  'scalar and pair radii keep their units and uniform SVG rectangles'() {
    ...
    assert.deepEqual(radius_of(pass.layout(new Rect({ border_radius: { tl: 0.1, tr: [0.1, em(0.5)] },
  ...
  'per-corner backgrounds, inside borders, and content clips share their outlines'() {
    const fragment = pass.layout(new Frame({ width: px(100), height: px(60), border_radius: { t: [px(20), px(10)] },
  ...
  'corner radii clamp, copy, and freeze at drawing and clipping boundaries'() {
    ...
    const thick = pass.layout(new Frame({ border_radius: { t: px(100) }, border_width: px(100), clip: true,
  ...
  'bars and BarPlot accept corner radii in screen coordinates while retaining scalar sizing'() {
    ...
    for (const radius of [0.1, [0.05, 0.1], { t: 0.1 }] satisfies RectRadius[]) {
      const fragment = pass.layout(new Graph({ xlim: [0, 2], ylim: [0, 2],
        children: new Bars({ values: [1], positions: [1], border_radius: radius }) }), fixed).children[0].fragment
      const r = { x: 10, y: 10 }
    ...
    for (const direction of ['vertical', 'horizontal'] as const) {
      ...
      const source = new BarPlot({ values: [1, NaN, -2], direction,
        styles: (value, index) => {
          ...
          return { fill: value > 0 ? 'blue' : 'red', border_radius: { [side]: px(4 + index) } }
        },
      })
  ...
  'per-bar radius overrides preserve shared defaults, explicit zero, and source snapshots'() {
    const custom = { t: px(6) }
    const styles = [{ border_radius: custom }, { border_radius: 0 }, { fill: 'red' }, { border_radius: undefined }]
    const bars = new Bars({ values: [1, 2, 3, 4], border_radius: px(8), styles })
    custom.t = px(30)
  ...
  'per-bar radii resolve layout units and clamp to each bar rectangle'() {
    const bars = new Bars({ values: [1, 1, 1], positions: [0, 1, 2],
      styles: [{ border_radius: em(0.5), font_size: px(20) }, { border_radius: [0.05, 0.1] }, { border_radius: px(1000) }],
    })
    ...
    const invalid = new Bars({ values: [1], styles: () => ({ border_radius: { t: px(-1) } }) })
  ...
  'invalid corner lengths, unknown keys, and malformed pairs fail clearly'() {
    for (const radius of [{ t: px(-1) }, { br: -0.1 }, { l: [px(2), px(-1)] },
      ...
      assert.throws(() => pass.layout(new Frame({ border_radius: radius as RectRadius }), fixed),
