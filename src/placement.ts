...
import { align_offset, layout_content, resolve_alignment } from './composition';
import type { Alignment } from './composition';
import { define_element, element_children, content_child } from './element';
import type { Element, ElementProps } from './element';
import { make_fragment, place_fragment, transform_guides } from './fragment';
import { make_size, make_point, make_rect, transform_rect } from './geometry';
import type { Transform, Size } from './geometry';
import { available, make_request, finish_size } from './layout';
...
const Overlay = define_element<OverlayProps>('Overlay', (props, query) => {
...
const TransformBox = define_element<TransformBoxProps>('TransformBox', (props, query) =>
  transformed_layout(props, query, () => props.matrix ?? [1, 0, 0, 1, 0, 0]));

const Rotate = define_element<RotateProps>('Rotate', (props, query) => transformed_layout(props, query, size => {
...
const Anchor = define_element<AnchorProps>('Anchor', (props, query) => {
  const child = content_child(props.children);
